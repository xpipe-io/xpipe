...
- The webtop has been completely overhauled and improved. It has been moved away from KasmVNC to Selkies, and upgraded to Ubuntu 26.04 with KDE6, which means that any old webtop envrionments can't be upgraded in place. Please create a new container with a new bind mounted volume for user data. You can also use the new webtop creation wizward from within XPipe for this. The Kasm Workspace setup is also no longer supported as this required a KasmVNC-based container. With this inconvenience out of the way, Selkies is a much better option for the webtop, which you can see for yourself.
...
- Some HTTP API json schemas and endpoints have changed, so you will have to check whether you API calls still work, and if not, adjust them a bit
...
- Make icons git repository clone not include entire git history to reduce size
