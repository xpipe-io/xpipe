...
public class StoreSectionMiniComp extends Comp<CompStructure<VBox>> {
    ...
    public CompStructure<VBox> createBase() {
        ...
        if (section.getWrapper() != null) {
            var root = new ButtonComp(section.getWrapper().nameProperty(), () -> {}).apply(struc -> struc.get().setGraphic(
                    PrettyImageHelper.ofFixedSmallSquare(
                                    section.getWrapper().getEntry().getProvider().getDisplayIconFileName(section.getWrapper().getEntry().getStore()))
                            .createRegion())).apply(struc -> {
                struc.get().setAlignment(Pos.CENTER_LEFT);
            }).grow(true, false).apply(struc -> struc.get().setMnemonicParsing(false)).styleClass("item");
            augment.accept(section, root);

            expanded = new SimpleBooleanProperty(section.getWrapper().getExpanded().get() && section.getShownChildren().size() > 0);
            var button = new IconButtonComp(
                    Bindings.createStringBinding(() -> expanded.get() ? "mdal-keyboard_arrow_down" : "mdal-keyboard_arrow_right", expanded), () -> {
                expanded.set(!expanded.get());
            }).apply(struc -> struc.get().setMinWidth(20)).apply(struc -> struc.get().setPrefWidth(20)).focusTraversable().accessibleText("Expand")
                    .disable(BindingsHelper.persist(Bindings.size(section.getAllChildren()).isEqualTo(0))).grow(false, true).styleClass(
                            "expand-button");
        ...
        var listSections = section.getWrapper() != null ?
                BindingsHelper.filteredContentBinding(section.getShownChildren(),
                        storeSection -> section.getAllChildren().size() <= 20 || expanded.get(), expanded, section.getAllChildren()) :
        ...
        list.add(new HorizontalComp(List.of(content)).styleClass("content").apply(struc -> struc.get().setFillHeight(true))
                .hide(BindingsHelper.persist(Bindings.or(Bindings.not(expanded), Bindings.size(section.getAllChildren()).isEqualTo(0)))));

        return new VerticalComp(list).styleClass("store-section-mini-comp").styleClass(section.getWrapper() != null ? "sub" : "top").apply(struc -> {
