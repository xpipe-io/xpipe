...
import io.xpipe.app.hub.entry.StoreQuickAccessButtonComp;
...
public abstract class StoreSectionBaseComp extends RegionBuilder<VBox> {
    ...
    protected void addPseudoClassListeners(VBox vbox, ObservableBooleanValue expanded) {
        ...
        if (section.getWrapper() != null) {
            if (section.getDepth() == 1) {
                Listeners.attachWithScene(vbox, section.getWrapper().getColor(), val -> {
            ...
            Listeners.attachWithScene(vbox, section.getWrapper().getAccessScopeRestricted(), val -> {
                vbox.pseudoClassStateChanged(PseudoClass.getPseudoClass("per-user"), val);
            });

            Listeners.attachWithScene(vbox, section.getWrapper().getTemplate(), val -> {
                vbox.pseudoClassStateChanged(PseudoClass.getPseudoClass("template"), val);
            });
        }
    }

    protected abstract boolean useGrayColorBox();
    ...
    protected RegionBuilder<Region> createTopDragIndicator() {
        return RegionBuilder.of(() -> {
            var top = new Region();
            top.getStyleClass().add("drag-indicator");
            Listeners.attachWithScene(top, StoreViewState.get().getSectionDragOperation(), storeSectionDrag -> {
                var active = storeSectionDrag != null
                        && storeSectionDrag
                                .getTarget()
                                .getSectionTarget()
                                .map(s -> s.equals(section))
                                .orElse(false);
                var topActive = active && storeSectionDrag.getTarget().getOrder() == StoreSectionDrag.Order.BEFORE;
                top.setVisible(topActive);
            });

            Listeners.attachWithScene(top, AppPrefs.get().condenseConnectionDisplay(), dense -> {
                top.pseudoClassStateChanged(PseudoClass.getPseudoClass("dense"), dense || section.getDepth() != 1);
            });
    ...
    protected RegionBuilder<Region> createBottomDragIndicator() {
        return RegionBuilder.of(() -> {
            var bottom = new Region();
            bottom.getStyleClass().add("drag-indicator");
            Listeners.attachWithScene(bottom, StoreViewState.get().getSectionDragOperation(), storeSectionDrag -> {
                var active = storeSectionDrag != null
                        && storeSectionDrag
                                .getTarget()
                                .getSectionTarget()
                                .map(s -> s.equals(section))
                                .orElse(false);
                var bottomActive = active && storeSectionDrag.getTarget().getOrder() == StoreSectionDrag.Order.AFTER;
                bottom.setVisible(bottomActive);
            });

            Listeners.attachWithScene(bottom, AppPrefs.get().condenseConnectionDisplay(), dense -> {
                bottom.pseudoClassStateChanged(PseudoClass.getPseudoClass("dense"), dense || section.getDepth() != 1);
            });
