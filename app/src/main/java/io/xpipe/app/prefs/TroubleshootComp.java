...
public class TroubleshootComp extends Comp<CompStructure<?>> {
    ...
    private Comp<?> createActions() {
        OptionsBuilder b = new OptionsBuilder().addTitle("troubleshootingOptions").spacer(13).addComp(
                        new TileButtonComp("reportIssue", "reportIssueDescription", "mdal-bug_report", e -> {
                            ...
                            e.consume();
                        }).grow(true, false), null).separator()
                //                .addComp(
                ...
                //                        null)
                //                .separator()
                .addComp(new TileButtonComp("launchDebugMode", "launchDebugModeDescription", "mdmz-refresh", e -> {
                    OperationMode.executeAfterShutdown(() -> {
                        try (var sc = new LocalStore().control().start()) {
                            var script = FileNames.join(XPipeInstallation.getCurrentInstallationBasePath().toString(),
        ...
        if (AppLogs.get().isWriteToFile()) {
            b.addComp(new TileButtonComp("openCurrentLogFile", "openCurrentLogFileDescription", "mdmz-text_snippet", e -> {
                FileOpener.openInTextEditor(AppLogs.get().getSessionLogsDirectory().resolve("xpipe.log").toString());
                e.consume();
            }).grow(true, false), null).separator();
        }

        b.addComp(new TileButtonComp("openInstallationDirectory", "openInstallationDirectoryDescription", "mdomz-snippet_folder", e -> {
            DesktopHelper.browsePath(XPipeInstallation.getCurrentInstallationBasePath());
            e.consume();
        }).grow(true, false), null).separator().addComp(new TileButtonComp("clearCaches", "clearCachesDescription", "mdi2t-trash-can-outline", e -> {
            ClearCacheAlert.show();
            e.consume();
        }).grow(true, false), null);
        return b.buildComp();
    }

    @Override
    public CompStructure<?> createBase() {
        var box = new VerticalComp(List.of(createActions())).apply(s -> s.get().setFillWidth(true)).apply(struc -> struc.get().setSpacing(15))
                .styleClass("troubleshoot-tab").apply(struc -> struc.get().setPrefWidth(600));
