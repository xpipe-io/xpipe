...
public class CustomFormRenderer extends PreferencesFxFormRenderer {
    ...
    public void initializeParts() {
        groups = form.getGroups().stream().map(group -> new PreferencesFxGroupRenderer((PreferencesFxGroup) group, this) {
            ...
            public void layoutParts() {
                ...
                List<Element> elements = preferencesGroup.getElements().stream().map(Element.class::cast).toList();
                ...
                for (int i = 0; i < elements.size(); i++) {
                    ...
                    if (element instanceof Field f) {
                        ...
                        descriptionLabel.opacityProperty().bind(c.getFieldLabel().opacityProperty().multiply(0.65));
