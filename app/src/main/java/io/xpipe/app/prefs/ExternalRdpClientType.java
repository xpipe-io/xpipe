...
public interface ExternalRdpClientType extends PrefsChoiceValue {
    ...
    default Path writeConfig(RdpConfig input) throws Exception {
        var file = Path.of(FileNames.join(
                LocalShell.getShell().getSystemTemporaryDirectory(), "exec-" + ScriptHelper.getScriptId() + ".rdp"));
        var string = input.toString();
        Files.writeString(file, string);
        return file;
    }

    ExternalRdpClientType MSTSC = new PathCheckType("app.mstsc", "mstsc.exe") {
    ...
    ExternalRdpClientType REMMINA = new PathCheckType("app.remmina", "remmina") {
    ...
    class CustomType extends ExternalApplicationType implements ExternalRdpClientType {
        ...
        public void launch(LaunchConfiguration configuration) throws Exception {
            ...
            ApplicationHelper.executeLocalApplication(
                    CommandBuilder.of().add(ApplicationHelper.replaceFileArgument(format, "FILE", writeConfig(configuration.getConfig()).toString())),
                    true);
        }
