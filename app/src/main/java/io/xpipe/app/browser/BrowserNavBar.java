...
public class BrowserNavBar extends SimpleComp {
    ...
    protected Region createSimple() {
        ...
        var pathBar = new TextFieldComp(path, true).styleClass(Styles.RIGHT_PILL).styleClass("path-text").apply(struc -> {
        ...
        var graphic = Bindings.createStringBinding(() -> {
            return model.getCurrentDirectory() != null ? FileIconManager.getFileIcon(model.getCurrentDirectory(), false) : "home_icon.svg";
        }, model.getCurrentPath());
        var breadcrumbsGraphic = PrettyImageHelper.ofSvg(graphic, 16, 16).padding(new Insets(0, 0, 1, 0)).styleClass("path-graphic").createRegion();
        ...
        var stack = new StackComp(List.of(pathBar, breadcrumbs)).apply(struc -> struc.get().setAlignment(Pos.CENTER_LEFT)).hgrow().apply(struc -> {
        ...
        var topBox = new HorizontalComp(List.of(Comp.of(() -> graphicButton), stack)).apply(struc -> struc.get().setAlignment(Pos.CENTER_LEFT)).apply(
                struc -> {
                    ((Region) struc.get().getChildren().get(0)).minHeightProperty().bind(((Region) struc.get().getChildren().get(
                            1)).heightProperty());
                    ((Region) struc.get().getChildren().get(0)).maxHeightProperty().bind(((Region) struc.get().getChildren().get(
                            1)).heightProperty());
                }).apply(struc -> {
