...
public class WindowDockComp<T extends WindowDockListener> extends SimpleRegionBuilder {
    ...
    protected void setupListeners(StackPane stack) {
        ...
        stack.sceneProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue == null) {
                var s = (Stage) oldValue.getWindow();
                ...
                var s = (Stage) newValue.getWindow();
    ...
    protected void update(Region region) {
        ...
        var handle = NativeWinWindowControl.byWindow(region.getScene().getWindow());
        if (handle.isEmpty()) {
            return;
        }
