...
public class SecretRetrievalStrategyHelper {
    ...
    private static OptionsBuilder inPlace(Property<SecretRetrievalStrategy.InPlace> p) {
        var secretProperty = new SimpleObjectProperty<>(p.getValue() != null ? p.getValue().getValue() : null);
        return new OptionsBuilder().addComp(new SecretFieldComp(secretProperty), secretProperty).bind(() -> {
    ...
    private static OptionsBuilder passwordManager(Property<SecretRetrievalStrategy.PasswordManager> p) {
        var keyProperty = new SimpleObjectProperty<>(p.getValue() != null ? p.getValue().getKey() : null);
        var content = new HorizontalComp(List.of(new TextFieldComp(keyProperty).apply(struc -> struc.get().setPromptText("Password key")).hgrow(),
                new ButtonComp(null, new FontIcon("mdomz-settings"), () -> {
                    AppPrefs.get().selectCategory(4);
                    App.getApp().getStage().requestFocus();
                }).grow(false, true))).apply(struc -> struc.get().setSpacing(10));
        return new OptionsBuilder().name("passwordKey").addComp(content, keyProperty).bind(() -> {
    ...
    private static OptionsBuilder customCommand(Property<SecretRetrievalStrategy.CustomCommand> p) {
        ...
        return new OptionsBuilder().name("command").addComp(content, cmdProperty).bind(() -> {
    ...
    public static OptionsBuilder comp(Property<SecretRetrievalStrategy> s) {
        ...
        var passwordManager = new SimpleObjectProperty<>(strat instanceof SecretRetrievalStrategy.PasswordManager i ? i : null);
        var customCommand = new SimpleObjectProperty<>(strat instanceof SecretRetrievalStrategy.CustomCommand i ? i : null);
        ...
        var selected = new SimpleIntegerProperty(strat instanceof SecretRetrievalStrategy.None ?
                0 :
                strat instanceof SecretRetrievalStrategy.InPlace ?
                        ...
                        strat instanceof SecretRetrievalStrategy.PasswordManager ?
                                ...
                                strat instanceof SecretRetrievalStrategy.CustomCommand ?
                                        3 :
                                        strat instanceof SecretRetrievalStrategy.Prompt ? 4 : strat == null ? -1 : 0);
        return new OptionsBuilder().choice(selected, map).bindChoice(() -> {
