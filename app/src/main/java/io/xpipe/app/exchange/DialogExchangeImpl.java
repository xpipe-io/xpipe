...
public class DialogExchangeImpl extends DialogExchange implements MessageExchangeImpl<DialogExchange.Request, DialogExchange.Response> {
    ...
    public static <T> DialogReference add(Dialog d, UUID uuid, FailableConsumer<T, Exception> onCompletion) throws Exception {
    ...
    public DialogExchange.Response handleRequest(BeaconHandler handler, Request msg) throws Exception {
        if (msg.isCancel()) {
            TrackEvent.withTrace("beacon", "Received cancel dialog request").tag("key", msg.getDialogKey()).handle();
        ...
        TrackEvent.withTrace("beacon", "Received normal dialog request").tag("key", msg.getDialogKey()).tag("value", msg.getValue()).tag("newElement",
                e).handle();
