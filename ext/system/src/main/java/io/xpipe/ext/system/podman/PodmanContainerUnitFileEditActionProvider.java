    ...
    public static class Action extends StoreAction<PodmanContainerStore> {
        ...
        public void executeImpl() throws Exception {
            ...
            if (!sc.view().fileExists(unitFile)) {
                throw ErrorEventFactory.expected(new IllegalArgumentException("Container unit file " + unitFileName + " not found at " + unitFile));
            }

            var content = sc.view().readTextFile(unitFile);
            var matcher = Pattern.compile("SourcePath=(.+)").matcher(content);
            if (!matcher.find()) {
                throw ErrorEventFactory.expected(new IllegalArgumentException("Source of container unit file " + unitFileName + " not locatable from file " + unitFile));
            }

            var sourceFile = FilePath.of(matcher.group(1));
            FileOpener.openString(sourceFile.getFileName(), sourceFile.getFileName(), sc.view().readTextFile(sourceFile), s -> {
                try {
