...
public class YubikeyPivStrategy implements SshIdentityStrategy {
    ...
    private String getFile(ShellControl parent) throws Exception {
        var file = switch (parent.getOsType()) {
            case OsType.Linux ignored -> "/usr/local/lib/libykcs11.so";
            ...
            case OsType.Windows ignored -> {
                var x64 = "C:\\Program Files\\Yubico\\Yubico PIV Tool\\bin\\libykcs11.dll";
                if (parent.getShellDialect().directoryExists(parent, x64).executeAndCheck()) {
                    yield x64;
                }

                var x86 = "C:\\Program Files (x86)\\Yubico\\Yubico PIV Tool\\bin\\libykcs11.dll";
                if (parent.getShellDialect().directoryExists(parent, x86).executeAndCheck()) {
                    yield x86;
                }
    ...
    public List<KeyValue> configOptions(ShellControl parent) throws Exception {
        return List.of(new KeyValue("IdentitiesOnly", "no"), new KeyValue("PKCS11Provider", getFile(parent)), new KeyValue("IdentityFile", "none"),
