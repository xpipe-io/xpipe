        ...
        for (BrowserEntry entry : entries) {
            ...
            switch (OsType.getLocal()) {
                case OsType.Windows windows -> {
                    ...
                    var content = parent != null ? String.format(
                            "$shell = New-Object -ComObject Shell.Application; $shell.NameSpace('%s').ParseName('%s').InvokeVerb('Properties')",
                            FileNames.getParent(e), FileNames.getFileName(e)) : String.format(
                            "$shell = New-Object -ComObject Shell.Application; $shell.NameSpace('%s').Self.InvokeVerb('Properties')", e);
