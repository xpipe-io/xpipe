...
public class XPipeUrlAction implements ActionProvider {
    ...
    public LauncherCallSite getLauncherCallSite() {
        return new XPipeLauncherCallSite() {
            ...
            public Action createAction(List<String> args) throws Exception {
                switch (args.get(0)) {
                    case "addStore" -> {
                        var storeString = DefaultSecretValue.builder().encryptedValue(args.get(1)).build();
                        var store = JacksonMapper.parse(storeString.getSecretValue(), DataStore.class);
                        return new AddStoreAction(store);
                    }
                    case "launch" -> {
                        var entry = DataStorage.get().getStoreEntryIfPresent(UUID.fromString(args.get(1))).orElseThrow();
                    ...
                    case "action" -> {
                        ...
                        var entry = DataStorage.get().getStoreEntryIfPresent(UUID.fromString(args.get(2))).orElseThrow();
    ...
    static class AddStoreAction implements ActionProvider.Action {
        ...
        public void execute() {
            ...
            var entry = DataStoreEntry.createNew(UUID.randomUUID(), StoreViewState.get().getActiveCategory().getValue().getCategory().getUuid(), "",
                    store);
